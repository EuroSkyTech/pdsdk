...
use substance::{
    BuildContext, BuildRunner, ByteSize, CrateName, DemangledSymbolWithoutHash, NumberOfCopies,
...
fn main() -> eyre::Result<()> {
    let path = std::env::current_exe()?
        .ancestors()
        .nth(3)
        .ok_or_eyre("no such directory")?
        .join("pdsdk")
        .join("Cargo.toml")
        .to_str()
        .ok_or_eyre("no such path")?
        .to_owned();

    // TODO: matrix through all features
    let ctx = BuildRunner::for_manifest(path).run()?;
